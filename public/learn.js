...
function initLearnLoom(pattern) {
    ...
    function createLearnHUD() {
        ...
        style.textContent = `
            ...
            #phaseTab_BEAT.active    { background:#003322; color:#00ff8        threading.push(customMap.length > 0 ? customMap[i % customMap.length] : (i % shaftCount));
    }
...
function initLearnLoom(pattern) {
    ...
    function skipCurrentPhase() {
        ...
        else if (currentPhase === "SHUTTLE" || currentPhase === "RELEASE_PEDAL") {
            if (currentPhase === "SHUTTLE") triggerShuttleThrow(); 
            // In a skip, just auto-clear pedals and jump to beat
            currentPressedPedals.clear();
            currentPhase = "BEAT";
            refreshHUD();
        }
    ...
    function goToPrevStep() {
        ...
        if (currentPhase !== "SHED") {
            if (currentPhase === "BEAT" || currentPhase === "SHUTTLE" || currentPhase === "RELEASE_PEDAL") {
                removeLastMistakeWeft();
            }
            currentPhase = "SHED";
            currentPressedPedals.clear();
