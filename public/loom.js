// ─────────────────────────────────────────────────────────────────────────────
// HABI-LIN loom.js  —  with per-event latency tracking
//
// WHAT'S NEW (search "LATENCY" to find every change):
//   1. LatencyTracker class  – stamps BLE arrival, handler entry, next RAF frame
//   2. handleHardwareInput   – calls tracker.onHandlerEnter(rawValue)
//   3. BLE characteristic    – calls tracker.onBLEArrival(rawValue) BEFORE decode
//   4. animate()             – calls tracker.onFrame() each RAF tick
//   5. createUI()            – injects a live latency panel into .loom-panel
//   6. processBeat()         – flushes the current event's log to /api/latency
//
// MOBILE UPDATE:
//   - Measurements and Latency panels are now accessible on mobile
//   - A floating pill bar at the bottom lets users toggle each panel as a drawer
// ─────────────────────────────────────────────────────────────────────────────
...
class LatencyTracker {
    constructor() {
        this._bleArrival   = null;
        this._handlerEntry = null;
        this._pendingLabel = null;
        this._frameNeeded  = false;
        this.history = [];
        this.MAX_HISTORY = 50;
        this._sumBleToHandler    = 0;
        this._sumHandlerToFrame  = 0;
        this._count              = 0;
        this.onUpdate = null;
    }

    onBLEArrival(rawLabel) {
        this._bleArrival   = performance.now();
        this._pendingLabel = rawLabel;
        this._handlerEntry = null;
        this._frameNeeded  = false;
    }

    onHandlerEnter(rawLabel) {
        if (this._bleArrival === null) {
            this._bleArrival   = performance.now();
            this._pendingLabel = rawLabel;
        }
        this._handlerEntry = performance.now();
        this._frameNeeded  = true;
    }

    onFrame() {
        if (!this._frameNeeded || this._handlerEntry === null) return;
        this._frameNeeded = false;

        const now             = performance.now();
        const bleToHandler    = this._handlerEntry - this._bleArrival;
        const handlerToFrame  = now - this._handlerEntry;

        const entry = {
            ts:             Date.now(),
            label:          this._pendingLabel || "unknown",
            bleToHandler:   Math.round(bleToHandler * 100) / 100,
            handlerToFrame: Math.round(handlerToFrame * 100) / 100,
            totalLatency:   Math.round((bleToHandler + handlerToFrame) * 100) / 100,
        };

        this.history.push(entry);
        if (this.history.length > this.MAX_HISTORY) this.history.shift();

        this._sumBleToHandler   += entry.bleToHandler;
        this._sumHandlerToFrame += entry.handlerToFrame;
        this._count++;
        ...
        if (this.onUpdate) this.onUpdate(entry);
    }

    get avgBleToHandler()   { return this._count ? this._sumBleToHandler   / this._count : 0; }
    get avgHandlerToFrame() { return this._count ? this._sumHandlerToFrame / this._count : 0; }
    get avgTotal()          { return this.avgBleToHandler + this.avgHandlerToFrame; }
    get lastEntry()         { return this.history[this.history.length - 1] || null; }

    recent(n = 10) { return this.history.slice(-n).reverse(); }

    async flush(patternId, patternName) {
        ...
        try {
            ...
            console.warn("Latency flush failed:", err);
        }
    }
}
...
function attachStartButton() {
    ...
    function buildColorUI() {
        ...
        if (mountEl) {
            ...
            if (startBtnEl && startBtnEl.parentNode) startBtnEl.parentNode.insertBefore(root, startBtnEl);
            else heddlesOverlay.appendChild(root);
        }
        ...
        root.querySelectorAll('.warp-toggle-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                root.querySelector('#cs-panel-warp-range').style.display = currentWarpMode === 'range' ? 'flex' : 'none';
                root.querySelector('#cs-panel-warp-heddle').style.display = currentWarpMode === 'heddle' ? 'flex' : 'none';
                drawWarpPreview();
            });
        });
        ...
        root.querySelectorAll('#cs-panel-warp-heddle input[type="color"]').forEach(inp => inp.addEventListener('input', drawWarpPreview));
        ...
        function drawWarpPreview() {
            ...
            for (let i = 0; i < total; i++) { ctx.fillStyle = colors[i]; ctx.fillRect(i, 0, 1, 1); }
        }
        ...
        function buildWarpColorArray(total) {
            ...
            if (currentWarpMode === 'range') {
                ...
                for (let i = 0; i < total; i++) arr[i] = colors[tempMap[i % patternSize] || 0];
            }
            return arr;
        }
        ...
        warpAddBtn.addEventListener("click", () => {
            ...
            const maxT = Math.floor(500 / Math.max(1, currentMax)) * currentMax;
            makeRow(nextFrom, maxT, "#000000");
        });

        makeRow(1, 500, "#ffffff");
    ...
    nextBtn.addEventListener("click", async (e) => {
        ...
        if (!patternName) { alert("Please enter a pattern name before proceeding."); return; }
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (isDuplicate) { alert("Pattern name already exists! Please choose a unique name."); return; }
            }
        } catch (err) { console.error("Could not verify pattern name uniqueness", err); }
        ...
        document.querySelectorAll(".range-to").forEach(el => {
            if (!el.value || el.value == "120") el.value = totalThreads;
        });
    ...
    startBtn.addEventListener("click", (e) => {
        ...
        if (isNaN(patternSize) || patternSize <= 0) { alert("Please enter a valid pattern size."); return; }
        ...
        if (allAssigned.length !== patternSize || uniqueAssigned.size !== patternSize) {
            alert(`Error: Assign exactly 1 to ${patternSize} without duplicates.`); return;
        }
        if (allAssigned.some(n => n < 1 || n > patternSize)) {
            alert(`Error: Thread numbers must be between 1 and ${patternSize}.`); return;
        }
...
function injectLoomStyles() {
    ...
    style.textContent = `
        .loom-panel { position:absolute; top:clamp(10px,2%,20px); left:clamp(10px,2%,20px); width:clamp(160px,18vw,220px); max-height:80vh; background:rgba(13,13,18,0.96); color:#e0e0e0; border:1px solid #2a2a35; border-radius:14px; padding:12px; display:flex; flex-direction:column; gap:10px; z-index:100; box-shadow:0 8px 32px rgba(0,0,0,0.55); font-family:'Plus Jakarta Sans',sans-serif; font-size:clamp(10px,1.1vw,12px); box-sizing:border-box; transition:all 0.3s ease; overflow-y:auto; }
        .loom-panel.collapsed { max-height:48px!important; min-height:48px!important; padding-bottom:0!important; overflow:hidden!important; }
        .loom-panel.collapsed > :not(.loom-panel__header) { display:none!important; }
        .loom-panel__header { display:flex; justify-content:space-between; align-items:center; padding-bottom:8px; border-bottom:1px solid #2a2a35; gap:6px; cursor:pointer; user-select:none; margin-bottom:0; }
        .loom-panel.collapsed .loom-panel__header { border-bottom:none; padding-bottom:0; height:100%; }
        .loom-panel__title { font-weight:700; font-size:clamp(11px,1.2vw,13px); color:#fff; letter-spacing:0.3px; flex:1; min-width:0; }
        .loom-btn-back { padding:4px 8px; background:none; border:1px solid #3a3a45; color:#888; border-radius:6px; cursor:pointer; font-size:clamp(9px,0.9vw,10px); font-family:inherit; transition:border-color 0.2s,color 0.2s; white-space:nowrap; flex-shrink:0; }
        .loom-btn-back:hover { border-color:#666; color:#ccc; }
        .loom-btn { padding:clamp(6px,1vh,9px) 8px; color:white; border:none; border-radius:8px; cursor:pointer; font-weight:700; font-size:clamp(10px,1vw,11px); width:100%; font-family:inherit; transition:opacity 0.2s,transform 0.1s; letter-spacing:0.2px; }
        ...
        .loom-hint { font-size:clamp(9px,0.95vw,10px); color:#666; line-height:1.4; background:#141418; padding:6px 8px; border-radius:6px; }
        .loom-divider { border:none; border-top:1px solid #2a2a35; margin:0; }
        .loom-color-row { display:flex; flex-direction:column; gap:6px; }
        .loom-color-label { display:flex; justify-content:space-between; align-items:center; font-size:clamp(10px,1vw,11px); color:#aaa; }
        .loom-color-label input[type="color"] { border:none; width:clamp(22px,2.5vw,28px); height:clamp(16px,2vh,20px); background:none; cursor:pointer; padding:0; border-radius:4px; }
        .loom-measure-panel { position:absolute; top:clamp(10px,2%,20px); right:clamp(10px,2%,20px); width:clamp(170px,16vw,210px); background:rgba(10,15,10,0.96); color:#e0e0e0; border:1px solid #1a2e1a; border-radius:14px; padding:12px; display:flex; flex-direction:column; gap:8px; z-index:100; box-shadow:0 8px 32px rgba(0,0,0,0.55); font-family:'Plus Jakarta Sans',sans-serif; font-size:clamp(10px,1.1vw,12px); box-sizing:border-box; }
        .loom-measure-panel__title { font-weight:700; font-size:clamp(11px,1.2vw,12px); color:#7edd8f; letter-spacing:0.5px; text-transform:uppercase; display:flex; align-items:center; gap:6px; padding-bottom:8px; border-bottom:1px solid #1a2e1a; }
        .loom-measure-panel__title::before { content:''; width:6px; height:6px; background:#4ecb5e; border-radius:50%; display:inline-block; animation:pulse-dot 1.5s ease-in-out infinite; }
        @keyframes pulse-dot { 0%,100%{opacity:1;transform:scale(1)} 50%{opacity:0.4;transform:scale(0.7)} }
        .measure-row { display:flex; justify-content:space-between; align-items:baseline; gap:4px; }
        .measure-label { font-size:clamp(9px,0.9vw,10px); color:#668866; white-space:nowrap; text-transform:uppercase; letter-spacing:0.4px; }
        .measure-value { font-size:clamp(11px,1.1vw,13px); font-weight:700; color:#c8f0cb; text-align:right; font-variant-numeric:tabular-nums; }
        .measure-value span { font-size:0.75em; color:#668866; font-weight:500; }
        .measure-divider { border:none; border-top:1px solid #1a2e1a; margin:2px 0; }
        .loom-pattern-panel { position:absolute; bottom:clamp(10px,2%,20px); right:clamp(10px,2%,20px); width:clamp(200px,28vw,420px); height:clamp(140px,22vh,260px); background:rgba(0,0,0,0.92); color:white; border-radius:12px; border:1px solid #2a2a35; padding:clamp(10px,1.5%,15px); box-sizing:border-box; z-index:101; display:flex; flex-direction:column; font-family:'Plus Jakarta Sans',sans-serif; box-shadow:0 8px 28px rgba(0,0,0,0.5); }
        .loom-pattern-panel__title { font-weight:700; font-size:clamp(11px,1.2vw,13px); margin-bottom:8px; color:#ddd; letter-spacing:0.3px; }
        .loom-pattern-panel__scroll { flex:1; overflow:auto; background:#111; padding:4px; border-radius:6px; scrollbar-width:thin; scrollbar-color:#333 transparent; }
        ...
        #patternCanvas { background:white; border-radius:3px; display:block; }
        @media (max-width:768px) { .hardware-sim-container{display:none!important} }
        @media (min-width:901px) { .loom-panel{width:clamp(200px,18vw,250px);max-height:50vh} .loom-pattern-panel{width:clamp(220px,30vw,450px);height:clamp(150px,25vh,300px)} .loom-measure-panel{width:clamp(180px,16vw,220px)} }
        @media (min-width:601px) and (max-width:900px) { .loom-panel{width:clamp(150px,20vw,200px);max-height:45vh} .loom-pattern-panel{width:clamp(180px,32vw,300px);height:clamp(120px,20vh,200px)} .loom-measure-panel{width:clamp(150px,18vw,190px)} }
        @media (max-width:600px) { .loom-panel{top:10px!important;left:10px!important;width:auto!important;min-width:180px;max-height:60vh!important;padding:10px!important} .loom-pattern-panel{bottom:0!important;right:0!important;left:0!important;width:100%!important;height:28vh!important;max-width:none!important;border-radius:16px 16px 0 0!important;border-bottom:none} #patternCanvas{width:100%!important;height:auto!important} }
        ...
        .lat-label { color: #4455aa; font-size: 0.85em; text-transform: uppercase; letter-spacing: 0.4px; }
        .lat-val   { font-weight: 700; color: #a0b8ff; font-variant-numeric: tabular-nums; }
        .lat-val span { font-size: 0.75em; color: #4455aa; }
        .lat-divider { border: none; border-top: 1px solid #1a1a40; margin: 2px 0; }
        .lat-badge {
        ...
        .lat-badge.good   { background: rgba(40,167,69,0.2);  color: #5ecf6e; border: 1px solid rgba(40,167,69,0.3); }
        .lat-badge.warn   { background: rgba(255,193,7,0.15); color: #ffd454; border: 1px solid rgba(255,193,7,0.3); }
        .lat-badge.bad    { background: rgba(220,53,69,0.2);  color: #ff7b87; border: 1px solid rgba(220,53,69,0.3); }
        .lat-event-list {
            display: flex;
            flex-direction: column;
            gap: 3px;
        ...
        .lat-event-name { color: #6677cc; }
        .lat-event-ms   { color: #8899dd; font-variant-numeric: tabular-nums; }

        /* ── MOBILE DRAWER SYSTEM ── */
        @media (max-width:768px) {
            .loom-measure-panel, .loom-latency-panel {
            ...
            .loom-measure-panel.mobile-open,
            .loom-latency-panel.mobile-open {
            ...
            .mobile-overlay-bg.visible { display: block; }
        }
        @media (min-width:769px) {
            .mobile-panel-bar { display: none !important; }
        }
    `;
...
function initLoom() {
    ...
    let zPositions = [];
    if (SHAFT_COUNT === 2) zPositions = [TOWER_Z - 0.25, TOWER_Z + 0.25];
    else zPositions = [TOWER_Z - 0.45, TOWER_Z - 0.15, TOWER_Z + 0.15, TOWER_Z + 0.45];
    ...
    let rowCounter       = 0;
    let weftColorHistory = (loomConfig.rowColors && Array.isArray(loomConfig.rowColors))
        ? Array.from(loomConfig.rowColors) : [];
    ...
    function createHeddleSystem() {
        ...
        for (let i = 0; i < SHAFT_COUNT; i++) {
            ...
            [-1, 1].forEach(side => {
                ...
                const rollerZ = i < 2 ? (zPositions[0] + zPositions[1]) / 2 : (zPositions[2] + zPositions[3]) / 2;
            ...
            for (let h = 0; h <= 110; h++) {
                ...
                hPoints.push(new THREE.Vector3(hx, FRAME_H / 2 - 0.05, 0), new THREE.Vector3(hx, -FRAME_H / 2 + 0.05, 0));
            }
            frameGroup.add(new THREE.LineSegments(new THREE.BufferGeometry().setFromPoints(hPoints), heddleWireMaterial));
    ...
    function createBeater() {
        ...
        beaterGroup.add(new THREE.Mesh(new THREE.BoxGeometry(HEDDLE_WIDTH, bh - 0.1, 0.05), reedMaterial));
    ...
    function createShuttle() {
        ...
        const cavity = new THREE.Mesh(new THREE.BoxGeometry(sWidth * 0.6, 0.12, 0.22), new THREE.MeshStandardMaterial({ color: 0x332211 }));
        cavity.position.y = 0.05;
        shuttleGroup.add(cavity);
        const quill = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, sWidth * 0.5, 12), new THREE.MeshStandardMaterial({ color: 0xeeddcc }));
        quill.rotation.z = Math.PI / 2;
        quill.position.y = 0.05;
        shuttleGroup.add(quill);
        const threadWrap = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.08, sWidth * 0.45, 12), shuttleThreadMaterial);
    ...
    function createPedals() {
        ...
        for (let i = 0; i < SHAFT_COUNT; i++) {
            ...
            const pedalLever = new THREE.Mesh(new THREE.BoxGeometry(0.3, 0.22, 4.6), woodMaterial);
    ...
    function createDynamicWarp() {
        for (let i = 0; i < TOTAL_THREADS; i++) {
            ...
            const thread = new THREE.Line(
                ...
                new THREE.LineBasicMaterial({ color: threadColor, opacity: 0.8, transparent: true })
            );
            scene.add(thread);
    ...
    function createClothBase() {
        for (let i = 0; i < TOTAL_THREADS; i++) {
            ...
            scene.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints(points), clothThreadMaterial));
        }
    }
    ...
    function handleHardwareInput(data) {
        ...
        if (pedalMapUp[data] !== undefined) { currentPressedPedals.delete(pedalMapUp[data]); return; }
        if (data.includes(",")) {
        ...
        if (data === "B_1" || data === "V") { isBeaterPulled = false; hasProcessedCurrentBeat = false; return; }
    ...
    async function connectBLE() {
        ...
        try {
            ...
            characteristic.addEventListener('characteristicvaluechanged', (e) => {
                ...
                const rawVal   = new TextDecoder().decode(rawBytes).trim();
                latency.onBLEArrival(rawVal);
                handleHardwareInput(rawVal);
            });
    ...
    window.addEventListener('keydown', (e) => {
        ...
        if (e.code === 'Space') { e.preventDefault(); shuttleCurrentSide === -1 ? handleHardwareInput("S1S2") : handleHardwareInput("S2S1"); }
    ...
    function addWeftThread() {
        ...
        scene.add(line);
        activeWeft = { line, isBeaten: false, live: true, valid: true, capturedShed: shed.slice(), warpPattern: rowStates.slice(), capturedColor: insertionColor };
    ...
    function qualityBadge(ms) {
        if (ms < 20)  return `<span class="lat-badge good">Good</span>`;
        if (ms < 60)  return `<span class="lat-badge warn">Fair</span>`;
        return `<span class="lat-badge bad">High</span>`;
    }

    function createLatencyPanel() {
        const panel = document.createElement("div");
        panel.className = "loom-latency-panel";
        panel.id = "loom-latency-panel";
        panel.innerHTML = `
            <div class="lat-title">Hardware Latency</div>
            <div class="lat-row">
                <span class="lat-label">BLE → Handler</span>
                <span class="lat-val" id="lat-ble">—<span> ms</span></span>
            </div>
            <div class="lat-row">
                <span class="lat-label">Handler → Frame</span>
                <span class="lat-val" id="lat-frame">—<span> ms</span></span>
            </div>
            <div class="lat-row">
                <span class="lat-label">Total</span>
                <span class="lat-val" id="lat-total">—<span> ms</span></span>
            </div>
            <div style="display:flex;align-items:center;gap:6px;">
                <span class="lat-label" style="flex:1;">Quality</span>
                <span id="lat-badge">—</span>
            </div>
            <hr class="lat-divider"/>
            <div class="lat-row">
                <span class="lat-label">Avg BLE→H</span>
                <span class="lat-val" id="lat-avg-ble">—<span> ms</span></span>
            </div>
            <div class="lat-row">
                <span class="lat-label">Avg H→Frame</span>
                <span class="lat-val" id="lat-avg-frame">—<span> ms</span></span>
            </div>
            <div class="lat-row">
                <span class="lat-label">Avg Total</span>
                <span class="lat-val" id="lat-avg-total">—<span> ms</span></span>
            </div>
            <hr class="lat-divider"/>
            <div class="lat-label" style="margin-bottom:4px;">Recent Events</div>
            <div class="lat-event-list" id="lat-event-list"></div>
        `;
        document.getElementById("weaving-studio").appendChild(panel);

        latency.onUpdate = (entry) => {
            document.getElementById("lat-ble").childNodes[0].textContent  = entry.bleToHandler.toFixed(2);
            document.getElementById("lat-frame").childNodes[0].textContent = entry.handlerToFrame.toFixed(2);
            document.getElementById("lat-total").childNodes[0].textContent = entry.totalLatency.toFixed(2);
            document.getElementById("lat-badge").innerHTML = qualityBadge(entry.totalLatency);
            document.getElementById("lat-avg-ble").childNodes[0].textContent   = latency.avgBleToHandler.toFixed(2);
            document.getElementById("lat-avg-frame").childNodes[0].textContent = latency.avgHandlerToFrame.toFixed(2);
            document.getElementById("lat-avg-total").childNodes[0].textContent = latency.avgTotal.toFixed(2);

            const list   = document.getElementById("lat-event-list");
            const recent = latency.recent(8);
            list.innerHTML = recent.map(r => `
                <div class="lat-event">
                    <span class="lat-event-name">${r.label.substring(0,10)}</span>
                    <span class="lat-event-ms">${r.totalLatency.toFixed(1)} ms</span>
                </div>`).join("");
        };
    }
    ...
    function createUI() {
        injectLoomStyles();
        document.querySelectorAll('.loom-panel, .loom-measure-panel, .loom-pattern-panel, .loom-latency-panel, .mobile-panel-bar, .mobile-overlay-bg').forEach(el => el.remove());
        ...
        gui.innerHTML = `
            ...
            <div id="manual-weft-box" class="loom-color-row">
                <label class="loom-color-label">Shuttle Color<input type="color" id="shuttleColor" value="#f0eadf"/></label>
            </div>
            <div id="range-weft-box" style="display:none;flex-direction:column;gap:6px;">
                ...
                <button id="addWeftRangeBtn" style="background:rgba(100,180,100,0.1);border:1px dashed rgba(100,180,100,0.35);color:#7ecf7e;padding:8px;border-radius:6px;cursor:pointer;font-size:10px;font-weight:600;">+ Add Pick Range</button>
            </div>
            <hr class="loom-divider" />
        ...
        strategySelect.addEventListener("change", (e) => {
            if (e.target.value === "range") {
                manualBox.style.display = "none"; rangeBox.style.display = "flex"; colorMode = "preset"; updatePresetWeftColors();
            } else {
                manualBox.style.display = "flex"; rangeBox.style.display = "none"; colorMode = "manual";
            }
        });
        ...
        function updatePresetWeftColors() {
            ...
            weftRangesContainer.querySelectorAll("div.w-row-item").forEach(row => {
                ...
                for (let i = from - 1; i < to; i++) { if (i < 2000) arr[i] = col; }
            });
        ...
        function addWeftControlRow(from, to, color) {
            ...
            div.style.cssText = "display:flex;justify-content:space-between;align-items:center;background:rgba(255,255,255,0.03);padding:6px 8px;border-radius:6px;border:1px solid rgba(255,255,255,0.08);";
            div.innerHTML = `
                <div style="display:flex;align-items:center;gap:6px;">
                ...
                <div style="display:flex;align-items:center;gap:10px;">
                    ...
                    <button class="w-del" style="background:rgba(217,48,37,0.15);border:1px solid rgba(217,48,37,0.3);color:#e06060;border-radius:4px;cursor:pointer;font-size:10px;width:22px;height:22px;display:flex;align-items:center;justify-content:center;">✕</button>
                </div>
            `;
            div.querySelector(".w-del").addEventListener("click", () => { div.remove(); updatePresetWeftColors(); });
        ...
        addWeftRangeBtn.addEventListener("click", () => {
            ...
            let nextFrom = 1;
            if (rows.length > 0) { const lastTo = parseInt(rows[rows.length - 1].querySelector(".w-to").value); if (!isNaN(lastTo)) nextFrom = lastTo + 1; }
            addWeftControlRow(nextFrom, nextFrom + 50, "#c0392b");
        });
        if (presetWeftColors.length === 0) addWeftControlRow(1, 200, "#f0eadf");
        if (colorMode === "preset") { strategySelect.value = "range"; strategySelect.dispatchEvent(new Event("change")); }

        // ── Measurement Panel ──
        const measurePanel = document.createElement("div");
        measurePanel.className = "loom-measure-panel";
        measurePanel.id = "loom-measure-panel";
        ...
        document.getElementById('weaving-studio').appendChild(overlayBg);

        const mobileBar = document.createElement('div');
        mobileBar.className = 'mobile-panel-bar';
        mobileBar.innerHTML = `
            <button class="mobile-panel-btn" data-target="loom-measure-panel">Measurements</button>
            <button class="mobile-panel-btn" data-target="loom-latency-panel">Latency</button>
        `;
        document.getElementById('weaving-studio').appendChild(mobileBar);

        function closeMobilePanels() {
            document.querySelectorAll('.loom-measure-panel, .loom-latency-panel')
                .forEach(p => p.classList.remove('mobile-open'));
            mobileBar.querySelectorAll('.mobile-panel-btn')
                .forEach(b => b.classList.remove('active'));
            overlayBg.classList.remove('visible');
        }

        mobileBar.querySelectorAll('.mobile-panel-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                const panel = document.getElementById(btn.dataset.target);
                const isOpen = panel && panel.classList.contains('mobile-open');
                closeMobilePanels();
                if (!isOpen && panel) {
        ...
        overlayBg.addEventListener('click', closeMobilePanels);

        // ── Event listeners ──
        document.getElementById('shuttleColor').addEventListener('input', (e) => {
            shuttleThreadMaterial.color.set(e.target.value);
            colorMode = "manual";
            if (activeWeft && !activeWeft.isBeaten) { activeWeft.capturedColor = e.target.value; activeWeft.line.material.color.set(e.target.value); }
            render2DPattern();
        });
        ...
        document.getElementById("savePattern").addEventListener("click", async () => {
            const initialCount  = (loomConfig.resumeHistory && Array.isArray(loomConfig.resumeHistory)) ? loomConfig.resumeHistory.length : 0;
            ...
            if (isImported) {
                if (newRowsAdded < 20) { alert(`ACCESS DENIED: Add at least 20 new rows. Progress: ${newRowsAdded}/20.`); return; }
            } else {
                if (currentCount < 20) { alert(`NOT ENOUGH DATA: Weave at least 20 rows. Current: ${currentCount}/20.`); return; }
            }
            ...
            const data = {
                ...
                isImported: false,
                isPrivate: loomConfig.isPrivate !== undefined ? loomConfig.isPrivate : true,
                measurements: {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    await latency.flush(loomConfig.patternId, loomConfig.patternName);
    ...
    function render2DPattern() {
        ...
        if (rowCount === 0 && (!activeWeft)) {
            ...
            ctx.fillStyle = "#111"; ctx.fillRect(0, 0, patternCanvas.width, patternCanvas.height);
            return;
        }
        ...
        if (activeWeft && activeWeft.live) {
            ...
            for (let ti = 0; ti < TOTAL_PHYSICAL_THREADS; ti++) {
                ...
                const x = ti * cellSize;
                const shaft = threading[baseIndex % threading.length] || 0;
                const warpCol = Array.isArray(warpColors) && warpColors.length > 4 ? (warpColors[baseIndex] || "#ffffff") : (warpColors[shaft] || "#ffffff");
    ...
    function exportPatternImage() {
        if (patternHistory.length === 0) { alert("Weave some rows first!"); return; }
        const baseWarpCount = patternHistory[0].length;
        const rowCount = patternHistory.length;
        const CELL_PX = 10; const PADDING = 60; const FOOTER_H = 140;
        ...
        ctx.beginPath(); ctx.moveTo(PADDING, footerTop); ctx.lineTo(canvasW - PADDING, footerTop); ctx.stroke();
        ctx.fillStyle = "#1a1a1a"; ctx.font = "bold 24px 'Plus Jakarta Sans', sans-serif"; ctx.textAlign = "left";
        ctx.fillText(loomConfig.patternName || "HABI-LIN Draft", PADDING, footerTop + 40);
        ctx.font = "14px 'Plus Jakarta Sans', sans-serif"; ctx.fillStyle = "#666666";
    ...
    function checkWeftInsertion() {
        ...
        if (crossingFromLeft || crossingFromRight) {
            if (weftThreads.length > 0 && !activeWeft) {
                ...
                if (isUndo) {
                    scene.remove(lastWeft.line); lastWeft.line.geometry.dispose(); lastWeft.line.material.dispose();
                    weftThreads.pop(); patternHistory.pop();
                    if (lastWeft.isBeaten) {
                        ...
                        if (colorMode === "preset" && presetWeftColors.length > 0) {
                            ...
                            if (prevColor) { shuttleThreadMaterial.color.set(prevColor); const el = document.getElementById("shuttleColor"); if (el) el.value = prevColor; }
                        }
                    }
                    fellZ = BEATER_HIT_Z - (rowCounter * ROW_SPACING);
                    shuttleInserted = true;
                    updateMeasurePanel(); render2DPattern(); return;
                }
            }
            addWeftThread();
            shuttleInserted = true;
    ...
    function checkDirectionChanges() {
        ...
        if (Math.abs(delta) > 0.01) {
            ...
            if (shuttleMovingPositive === null) shuttleMovingPositive = movingPositive;
    ...
    function processBeat(currentHitZ) {
        ...
        rowCounter++;
        const committedColor = activeWeft.capturedColor || "#" + shuttleThreadMaterial.color.getHexString();
        weftColorHistory.push(committedColor);
        if (activeWeft.line && activeWeft.line.material) activeWeft.line.material.color.set(committedColor);
        fellZ = currentHitZ;
        activeWeft = null;
        weftReadyToBeat = false;
        shuttleDirectionChanges = 0;
        ...
        if (colorMode === "preset" && presetWeftColors.length > 0) {
            const nextColor = presetWeftColors[weftCount] || presetWeftColors[presetWeftColors.length - 1];
            if (nextColor) { shuttleThreadMaterial.color.set(nextColor); const el = document.getElementById("shuttleColor"); if (el) el.value = nextColor; }
        }

        const lastEntry = latency.lastEntry;
        if (lastEntry) {
            fetch("/api/latency/event", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    patternId:   loomConfig.patternId || null,
                    patternName: loomConfig.patternName,
                    event:       lastEntry,
                }),
            }).catch(() => {});
        }
    ...
    function reconstructSavedWeave() {
        ...
        loomConfig.resumeHistory.forEach((rowStates, index) => {
            ...
            const line = new THREE.Line(new THREE.BufferGeometry().setFromPoints(points), new THREE.LineBasicMaterial({ color: rowColor }));
            scene.add(line);
    ...
    if (loomConfig.resumeHistory && loomConfig.resumeHistory.length > 0) reconstructSavedWeave();
